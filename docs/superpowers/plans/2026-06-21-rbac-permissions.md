...
export const PERMISSIONS: PermissionDef[] = [
  ...
  { key: 'competition.sync', label: '全量同步飞书场景数据', group: '场景数据' },
...
> 2026-06-26 当前状态：场景数据写入型同步已统一收口到 `POST /api/admin/competition-sync`，由 `competition.sync` 控制；`/api/wish-pool/sync` 和 `/api/wish-pool/sync-field-map` 已删除，`/api/competitions/sync` 仅保留 GET 只读兼容。以下原计划保留作历史记录，不再作为当前实施指引。
